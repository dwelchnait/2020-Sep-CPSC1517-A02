...
namespace WebApp.NorthwindPages
{
    ...
    {
        //this is needed for the DataList error handling code
        List<string> errormsgs = new List<string>();

        protected void Page_Load(object sender, EventArgs e)
        {
            //clear out old messages fromt he DataList control (error handling)
            Message.DataSource = null;
            Message.DataBind();

            //do on initial load of page
            if (!Page.IsPostBack)
            {
                BindCategoryList();
                BindSupplierList();
        ...
        {
            //checking if you are on the prompt line (if present) of the dropdownlist
            //   is the ONE PLACE I will use .SelectedIndex
            //the PHYSICAL position of the prompt line is index = 0
